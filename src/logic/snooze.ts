 ...
 *              `opts.includeNow` is true, a dateless `now` option (land the task
 *              in the Now view — clear the due date, and eject it from the Someday
 *              list when it's parked there) is appended after the date options;
 *              its `date` is null. It sits just before `someday` (both are
 *              dateless; they differ only in where the task lands).
 */
export function computeSnoozeOptions(
  today: Date = new Date(),
  opts?: { includeToday?: boolean; includeSomeday?: boolean; includeNow?: boolean },
): SnoozeOption[] {
  ...
  if (opts?.includeNow) {
    options.push({ key: 'now', label: 'Now', date: null });
  }
